...
"""
Early Haar featuress by Viola and Jones (look at Haar feature tips issue #1)
"""
#------------------------------------------------------------------------------------------------------------------------------------------------
...
#(3) but vertical divisions
def rect3_haar_vert(gray_img,lenh,lenw,stride_h=1,stride_w=1):
    feature_dict={}
    (h,w)=gray_img.shape
    for i in range(0,h-lenh+1,stride_h):
        for j in range(0,w-lenw+1,stride_w):
            temp=gray_img[i:i+lenh,j:j+lenw]
            temp_int=integral_image(temp)
            bsum1=block_sum(temp_int,(0,0),lenw,int(lenh/3))
            bsum2=block_sum(temp_int,(0,int(lenh/3)),lenw,int(lenh/3))
            bsum3=block_sum(temp_int,(0,int((lenh*2)/3)),lenw,int(lenh/3))
            res=bsum2-(bsum1+bsum3)
            feature_dict['rect3_hort_'+str(lenh)+'x'+str(lenw)+'_'+'('+str(i)+','+str(j)+')']=res
    return feature_dict
...
if __name__ == "__main__":
    ...
    img=cv2.imread("facial_images\\0.png")
    print(img.shape)
    cv2.imshow("Img",img)
    cv2.waitKey(0)
    ...
    for i in range(3,h24+1):
        for j in range(1,w24+1):
            features.update(rect3_haar_vert(gray24,i,j,i,j))
